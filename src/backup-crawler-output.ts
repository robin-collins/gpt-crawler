...
async function backupCrawlerOutput() {
  try {
    ...
    async function getUniqueFilename(filepath: string, prefix?: string): Promise<string> {
    ...
    const uniqueConfigPath = await getUniqueFilename(configDestPath, `${domainName}-${defaultConfig.outputFileFormat}`);

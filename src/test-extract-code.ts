...
const filePath = 'D:/projects/gpt-crawler/save_output/initial_page_content.html';
 ...
 */
function extractCodeFromTabs(htmlContent: string): Record<string, string> | null {
  try {
    ...
    const tabTriggers = document.querySelectorAll('div[role="tablist"] button[role="tab"]');

    if (tabTriggers.length === 0) {
      console.warn('No tab triggers found. Check the selectors or HTML structure.');
    ...
    tabTriggers.forEach((triggerButton) => {
      ...
      if (!filename) {
        console.warn('Found a tab trigger button without text content (filename). Skipping.');
        return; // Skip buttons without text
      }

      if (!controlsId) {
        console.warn(`Tab trigger "${filename}" is missing the 'aria-controls' attribute. Skipping.`);
        return; // Skip buttons without the control link
      }

      // Find the corresponding content panel using the ID from aria-controls
      const contentPanel = document.querySelector(`div[role="tabpanel"]#${controlsId}`);

      if (!contentPanel) {
        console.warn(`Could not find content panel for tab "${filename}" with ID "${controlsId}". Skipping.`);
        return; // Skip if content panel not found
      }

      // Find the code block within the panel (look inside <pre> then <code>, or just <pre>)
      const codeElement = contentPanel.querySelector('pre code') || contentPanel.querySelector('pre');

      if (!codeElement) {
        console.warn(`Could not find code block (<pre><code> or <pre>) within panel for tab "${filename}". Skipping.`);
      ...
      if (sourceCode !== null && sourceCode.trim() !== '') {
        ...
        console.warn(`Extracted empty or null source code for: ${filename}. Skipping.`);
      }
    });
...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(absoluteOutputPath, JSON.stringify(codeData, null, 2), 'utf-8');
